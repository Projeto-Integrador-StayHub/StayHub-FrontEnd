...
const AdminPainelReserva = () => {
    ...
    const handleSaveEditEdit = async (updatedEdit: any) => {
        try {
            const response = await fetch(
                `https://localhost:7274/api/Reserva/EditarReserva/${updatedEdit.id}`,
                {
                    method: "PUT",
                    headers: {
                        "Content-Type": "application/json",
    ...
    const handleDeleteEdit = async (id: number) => {
        if (confirm("Tem certeza que deseja excluir este perfil?")) {
            try {
                const response = await fetch(`https://localhost:7274/api/Reserva/ExcluirReserva/${id}`, {
                    method: "DELETE",
                    headers: {
                        "Content-Type": "application/json",
                    },
                            ...
                            {filteredEdit.length > 0 ? (
                                filteredEdit.map((userEdit) => (
                                    <tr className={style.containerInfo} key={userEdit.id}>
                                        <td >{userEdit.nome}</td>
                                        <td>{userEdit.entrada}</td>
                                        <td>{userEdit.saida}</td>
                                        <td>{userEdit.preco}</td>
...
const EditModalEdit = ({ userEdit, onSave, onCancel }: any) => {
    ...
    const [cpf, setCpf] = useState(userEdit.cpf);
    const [telefone, setTelefone] = useState(userEdit.telefone);
    const [nascimento, setNascimento] = useState(userEdit.nascimento);
    const [senha, setSenha] = useState(userEdit.senha);
    const [endereco, setendereco] = useState(userEdit.endereco);
    const [mostrarSenha, setMostrarSenha] = useState(false);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onSave({ ...userEdit, nome: name, email, cpf, telefone, nascimento, senha, endereco });
    };
...
export default AdminPainelReserva ;